    ...
    while run != "stop" and (not rospy.is_shutdown()):
        ...
        if case_traj == "1":
            ...
            if (t-wait) <= np.arccos(1 - 4 * p1) / omega3:
                ...
                hum_pose_msg.pose.position.y = initial_pose.position.y + R - amp_traj3 - \
                    np.sqrt(R**2 - (reference_pose.position.x - initial_pose.position.x - p1 * x5)**2)
                reference_pose.position.y    = initial_pose.position.y + R - amp_traj3 - \
                    np.sqrt(R**2 - (reference_pose.position.x - initial_pose.position.x - p1 * x5)**2)
                ...
                hum_pose_msg.pose.position.y = initial_pose.position.y - R + amp_traj3 + \
                    np.sqrt(R**2 - (reference_pose.position.x - initial_pose.position.x - 0.5*x5 - R)**2)
                reference_pose.position.y = initial_pose.position.y - R + amp_traj3 + \
                    np.sqrt(R**2 - (reference_pose.position.x - initial_pose.position.x - 0.5*x5 - R)**2)
